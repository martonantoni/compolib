...
struct cDirection
{
    ...
    {
        return cDirection((int)"^>V<"s.find(c == 'v' ? 'V' : c) * 2);
    }
...
inline cImage<char> loadImageStdIn(int rows, int cols)
{
    cImage<char> image;
    image.w = cols;
    image.h = rows;
    int size = image.w * image.h;
    image.cells.resize(size + 1);
    int p = 0;
    for (int r = 0; r < rows; ++r)
    {
        char* line = image.cells.data() + p;
        scanf("%s", line);
        p += image.w;
    }
    return image;
...
int floodFillImage(cImage<char>& img, cPosition startPos, char fillChar, char borderChar)
// borderChar: what is considered as the border
{
    vector<cPosition> f;
    f.emplace_back(startPos);
    int count = 0;
    while (!f.empty())
    {
        auto p = f.back();
