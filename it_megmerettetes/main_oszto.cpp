...
const size_t pregen_p_s = 10'000'001;
...
void InitPrimes()
{
    ...
    for (ll i = 2; i < pregen_p_s; ++i)
    {
        if (!is_not_prime[i])
        {
            primes.push_back(i);
            pss.emplace_back(i * i);
            for (ll j = i * 2; j < pregen_p_s; j += i)
            {
                is_not_prime[j] = 1;
            }
        }
...
{
    InitPrimes();
    for (const auto& entry : fs::directory_iterator("oszto"))
    {
