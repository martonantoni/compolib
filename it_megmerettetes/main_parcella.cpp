...
//    Ellipse e(cx, cy, a, b);
    ...
    double bs = b * b;
    ll count = 0;
    for (double y = top - 0.5; y >= 0.5; y -= 1.0)
    {
        double rx = a * sqrt(1 - (y * y) / bs);
//         printf("at Y %.3f; from %.3f -> %.3f\n", y, -rx, rx);
        double r_count = floor(rx + 0.5);
//         printf("at Y %.3f; from %.3f -> %.3f, count: %.3f\n", y, -rx, rx, r_count);
        count += r_count * 4;

    }
    printf("total: %lld\n", count);

//    printf("Count: %lld", e.countllegerVertices());
}
...
int main()
{
    ...
    for (const auto& entry : fs::directory_iterator("parcella"))  // <<<<--------------------------------------- folder name
    {
